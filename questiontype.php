...
//        $image = get_question_image($question);
    ...
    function compute_feedbackperconditions(&$question, &$state) {
    	...
    	foreach ($parameters as $param) {
    		...
    		$param = $paramparts[0];
    		$state->responses[$param] = (isset($state->responses[$param])) ? ($state->responses[$param]) : (NULL);
    		$givenanswer[$param] = optional_param("resp".$question->id."_".$param, $state->responses[$param], PARAM_RAW);
    	}
    ...
    function check_condition($condition, $parameters, &$values, $question) {
        global $CFG, $USER;
        //if ($USER->id == 1064) { echo '<pre>'; print_r($condition);echo "\n"; print_r($question->givenanswer); }
        $values = '';
        $interval = false;
...
//        include_once("{$CFG->dirroot}/lib/evalmath/evalmath.class.php");//question/type/multinumerical/evalmath.class.php");
        $math = new EvalMath();
...
//        mail('nico@kolya.ch', 'DEBUG', 'return('.$leftvalue.$operator.$rightvalue.');');
